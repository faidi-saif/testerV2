...
class CameraMind :
    ...
    def get_results(self,arg_result_path):
        ...
        if self.vcamera.is_ready('ssh','serial',arg_timeout=50):
            ...
            self.vcamera.clean_content('/tmp/fuse_d/DCIM/100GOPRO/*')
            self.get_debug_dump_results(get_parent_path(arg_result_path) + '/DUMPSTITCH')
            self.get_dump_raw(get_parent_path(arg_result_path) + '/BAYER'  )
        else :
            raise Exception('camera is not ready , timeout')

    # ---------------------------------------------- get_debug_dump_results ------------------------------------------
    def get_debug_dump_results(self,arg_path):
        '''
        ...
        if self.vcamera.is_ready('ssh', 'serial', arg_timeout=50):
            if self.conf.options_mode['debug_dump'] == 'enable':
                self.vcamera.get_files('/tmp/fuse_d/DUMPSTITCH/', arg_path)
    ...
    # ---------------------------------------------- get_dump_raw ------------------------------------------
    def get_dump_raw(self, arg_path):
        '''
        ...
        if self.vcamera.is_ready('ssh', 'serial', arg_timeout=50):
            if self.conf.options_mode['dump_raw'] == 'enable':
                self.vcamera.get_files('/tmp/fuse_d/BAYER/', arg_path)
    ...
    def post_still_cmds(self):
        ...
        post_still = post_still+['sleep 5','t frw test stop_still']
        return  post_still
    ...
    def get_videos_cmds(self):
        ...
        cmd = cmd + ['sleep {}'.format(self.conf.options_mode['run_time']),'t frw test stop_video']
        return cmd
